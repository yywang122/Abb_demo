...
def display_instances(image_ori, boxes, masks, ids, names, scores):
    ...
    for i, color in enumerate(colors):
        ...
        if abs(x1-x2)<=50 and abs(y1-y2)<50:
            ...
            dis_t=round(dis * 100, 2)
            print("center:",centerx,centery)
            print("distance:",dis)
            print('distance_tttttt:',dis_t)
            ...
            print("camera_coordinate[1]:",camera_coordinate[1])
            print("camera_coordinate[2]:",camera_coordinate[2])
            #RX=float(1090*camera_coordinate[1]+477)
            #RY=float(937.5*camera_coordinate[0]-38.125)
            #RZ=float(-9.6542*dis_t+1401.6)
            RX=float(970.51*camera_coordinate[1]+482.16222)
            RY=float(991.56*camera_coordinate[0]-37.83813)
            RZ=float(-9.6542*dis_t+1401.6)
            if RZ < 490:
                RZ=490
            ...
            print("sorted objects:",RX,RY,RZ)
            # print(type(object),type(object[0][3]))
            # print(RX,RY)
            # print(position_safe(RX,RY))
            if position_safe(RX,RY):
                R.set_cartesian([[RX, RY, RZ], [0,0,1,0]])
                time.sleep(1)
                ...
                #time.sleep(3)
                #up  
                #R.set_cartesian([[RX, RY, 590], [0,0,1,0]])
                #time.sleep(10)
            '''
            if position_safe(RX,RY):
                R.set_cartesian([[RX, RY, RZ], [0,0,1,0]])
                ...
                R.set_cartesian([[RX, RY, 590], [0,0,1,0]])
                time.sleep(2)
                R.set_cartesian([[264.88, -10.7, 600], [0,0,1,0]])
                time.sleep(2)
                # Opening calibration
                ...
                #down
                R.set_cartesian([[RX, RY, 490], [0,0,1,0]])
                time.sleep(2)
                # Opening calibration
                ser.write(serial.to_bytes([0x48, 0x49, 0x74, 0x01, 0x01, 0xA0, 0x01, 0x55, 0Xc6]))
                time.sleep(2)
